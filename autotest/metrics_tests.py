...
def res_and_ens_test():
    ...
    mod = obs+pyemu.en.rng.standard_normal((200,))*.5    
    ...
    pyemu.en.rng = pyemu.en.rng.default_rng(98)
    res.weight =  [float(i>.5) for i in pyemu.en.rng.random(200)]

    # and an ensemble version
    ens = pd.DataFrame(np.tile(obs,(10,1))+pyemu.en.rng.standard_normal((10,200,))*.5, columns=obsnames)
    ens.loc['base'] = mod

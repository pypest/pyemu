...
def additive_parameter_interaction(d_vars, pars):
    ...
    return 1 / len(pars) * np.array([np.sum(pars[even]), np.sum(pars[odd])])
...
class IOWrapper:
    ...
    def parse():
        ...
        if not(args.stochastic is None or args.stochastic.lower() in parameter_interactions.keys() or
               args.stochastic.lower() == 'none'):

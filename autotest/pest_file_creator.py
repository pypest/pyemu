...
def setup_files(name, parameter_bounds, par_interaction):
    ...
    num_parameters = prob.number_decision_variables()
    create_template(prob, name, num_parameters)
    create_instruction_file(prob, name)
    pstfile = create_pest_file(prob, name, num_parameters, parameter_bounds, par_interaction)
    # run pest check and return pest control file
    # create in problem suite
    os.chdir('..')
    for f in os.listdir():
        if f.startswith(name):
            os.remove(f)
